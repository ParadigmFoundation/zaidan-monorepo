...
class RedisInterface():
    ...
    def set_unhedged_position(self, symbol: str, size: float) -> None:
        ...
        size_val = str(size)
        self.redis_database.hset(self.unhedged_position_key, symbol.upper(), size_val)
    ...
    def get_unhedged_position(self, symbol: str) -> float:
        ...
        if not self.redis_database.hexists(self.unhedged_position_key, symbol.upper()):
            return 0.0

        str_val = self.redis_database.hget(self.unhedged_position_key, symbol.upper())
